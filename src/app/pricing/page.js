...
const FAQ_ITEMS = [
  {
    ...
    a: <>Your credit balance is checked and deducted <strong className="text-[#e8e8ed] font-medium">at the moment you generate</strong> a note or run transcription. If your balance is too low, <strong className="text-[#e8e8ed] font-medium">the generation won't start</strong> — you'll see an error asking you to top up first.</>,
  },
  {
    q: 'Do credits expire?',
    a: <>No. Credits <strong className="text-[#e8e8ed] font-medium">never expire</strong> and carry over indefinitely.</>,
  },
  {
    q: 'How does group billing work?',
    a: <>Costs are split automatically across all group members. If anyone lacks sufficient credits, the generation won't run. The member who clicks generate gets a <strong className="text-[#e8e8ed] font-medium">50% discount</strong> on their share, subsidized by the rest of the group.</>,
  },
  {
    q: 'Why do note costs vary? What determines my tier?',
    a: <>Notes are charged based on <strong className="text-[#e8e8ed] font-medium">total token count</strong> (input transcript + AI output combined). Longer lectures with denser content produce more tokens. A typical 2–3 hour lecture usually falls in <strong className="text-[#e8e8ed] font-medium">Tier 2 (17 credits)</strong>.</>,
  },
  {
    q: 'Why is transcription priced separately from notes?',
    a: <>Transcription uses Whisper (priced by audio duration), while notes use a language model priced by token count. They run independently — if you already have a transcript, <strong className="text-[#e8e8ed] font-medium">you skip transcription cost entirely</strong>.</>,
  },
  {
    q: 'How do I top up?',
    a: <>Top up with any major credit or debit card via Stripe. Credits are added <strong className="text-[#e8e8ed] font-medium">instantly</strong> after payment confirmation.</>,
  },
];
...
function Cr({ amount, size = 13, iconSize = 12, className = '' }) {
  return (
    <span className={`inline-flex items-center gap-1 font-mono font-medium text-[#00d4c8] ${className}`} style={{ fontSize: size }}>
      {amount}
...
function SectionLabel({ children }) {
  return (
    <div className="mb-2.5 text-[10px] uppercase tracking-[0.1em] text-[#9a9aaa] opacity-90 select-none">
      {children}
    </div>
  );
}
...
function TierRow({ t, model }) {
  ...
  return (
    ...
    >
      <div className="text-[12.5px] text-[#b4b4c2]">
        {t.strong
      ...
      <span className="w-[90px] flex items-center justify-end gap-1 font-mono text-[13px] font-medium text-[#00d4c8]">
        {isT4 ? (
          <><Cr amount={perHr} size={13} iconSize={12} /><span className="text-[#9a9aaa] font-normal text-[11px]">/hr</span></>
        ) : (
...
function FaqItem({ q, a }) {
  ...
  return (
    <button onClick={() => setOpen(v => !v)} className="w-full text-left bg-[#111116]">
      <div className={`flex items-center justify-between gap-3 px-[18px] py-3.5 text-[13px] transition-colors ${open ? 'text-[#e8e8ed]' : 'text-[#b4b4c2] hover:text-[#e8e8ed]'}`}>
        <span>{q}</span>
        <svg viewBox="0 0 24 24" className={`h-3.5 w-3.5 flex-shrink-0 stroke-[#9a9aaa] fill-none stroke-2 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>
          <polyline points="6 9 12 15 18 9" />
        </svg>
      </div>
      <div className={`overflow-hidden transition-all duration-200 ${open ? 'max-h-96 pb-3.5' : 'max-h-0'}`}>
        <p className="px-[18px] text-left text-[13px] leading-[1.7] text-[#9a9aaa]">{a}</p>
      </div>
    </button>
  );
}
...
function BalanceInput({ label, value, onChange }) {
  return (
    <div className="flex flex-col gap-1.5">
      <div className="text-[11px] uppercase tracking-[0.06em] text-[#9a9aaa] opacity-70">{label}</div>
      <div className="flex overflow-hidden rounded-lg border border-white/[0.07] focus-within:border-[rgba(0,212,200,0.35)] transition-colors">
        <div className="flex items-center border-r border-white/[0.07] bg-[#1e1e27] px-3 py-2">
...
export default function Pricing() {
  ...
  if (calcTab === 'notes') {
    ...
    calcTierRange = durIdx === 3
      ? `${exactHours} hours (${calcTxModel === 'prem' ? 11 : 7}/hr)`
        ...
        <main className="flex flex-1 flex-col overflow-hidden min-w-0">
          ...
          >
            ...
            <motion.div variants={itemVariants}>
              ...
              <p className="text-[13px] text-[#9a9aaa]">Credit-based — top up once, spend as you go. <span className="text-[#e8e8ed] font-medium">No subscription. No expiry.</span></p>
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <TierBlock
                  title="Inclass Notes"
                  headerRight={<span className="text-[11px] text-[#9a9aaa]">per generation</span>}
                  tiers={NOTE_TIERS}
                  icon={<svg viewBox="0 0 24 24" className="h-3.5 w-3.5 stroke-[#00d4c8] fill-none stroke-[1.8] mr-2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>}
                />
                <TierBlock
                  title="Transcription"
                  model={txModel}
                      ...
                      {['turbo', 'prem'].map((m) => (
                        <button key={m} onClick={() => setTxModel(m)}
                          className={`rounded px-2.5 py-0.5 text-[10px] uppercase tracking-[0.05em] font-medium transition-all
                            ${txModel === m ? 'bg-[#18181f] text-[#00d4c8] border border-white/[0.07]' : 'text-[#9a9aaa] hover:text-[#b4b4c2]'}`}>
                  ...
                  icon={<svg viewBox="0 0 24 24" className="h-3.5 w-3.5 stroke-[#00d4c8] fill-none stroke-[1.8] mr-2"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></svg>}
                />
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
                    ...
                    <div key={group.name} className="overflow-hidden rounded-xl border border-white/[0.07] bg-[#111116] flex flex-col surface noise">
                      <div className="flex items-center justify-between border-b border-white/[0.07] px-[18px] py-3.5">
                        ...
                        <div className="flex flex-col items-end gap-0.5">
                          <span className="text-[11px] text-[#9a9aaa]">≤ {group.members} members</span>
                      ...
                      {group.tiers.map((t, i) => (
                        <div key={i} className="flex items-center justify-between border-b border-white/[0.05] px-[18px] py-2 last:border-0 hover:bg-white/[0.015] transition-colors">
                          ...
                          <span className="flex-1 mx-3 text-[11.5px] text-[#9a9aaa]">{t.range}</span>
                          <Cr amount={t.price} size={13} iconSize={12} />
                        </div>
                      ))}
                      <div className="border-t border-white/[0.05] px-[18px] py-3 bg-white/[0.01] mt-auto">
                        <div className="text-[10.5px] font-medium uppercase tracking-[0.05em] text-[#b4b4c2] mb-1.5 opacity-90">T2 Split Example</div>
                        <div className="flex flex-col gap-1 text-[11px] text-[#9a9aaa]">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className="inline-flex items-center gap-1 font-mono text-[12px] text-[#e8e8ed]">
              ...
              <p className="mt-2.5 text-[11.5px] text-[#9a9aaa] opacity-90">
                Group generation costs are automatically split across all members' credit balances. The member who clicks generate pays <span className="text-[#e8e8ed] font-medium">50% less</span> to incentivize organizing.
              </p>
            </motion.div>
                    ...
                    {['notes', 'transcript', 'pipeline'].map((tab) => (
                      <button key={tab} onClick={() => setCalcTab(tab)}
                        className={`rounded-md px-3.5 py-[5px] text-[12px] transition-all duration-150
                          ${calcTab === tab ? 'bg-[#18181f] text-[#e8e8ed] border border-white/[0.07]' : 'text-[#9a9aaa] hover:text-[#b4b4c2]'}`}>
                ...
                <div className="grid grid-cols-2 gap-6 p-[18px]">
                  <div className="flex flex-col gap-4">
                    ...
                    {calcTab === 'notes' && (
                      <>
                        <div className="flex flex-col gap-2">
                          <div className="text-[11px] uppercase tracking-[0.06em] text-[#9a9aaa] opacity-70">Estimated token count</div>
                          <div className="text-[11px] text-[#9a9aaa] opacity-85">Typical 3-hr lecture ≈ <span className="text-[#b4b4c2] font-medium">20k–28k tokens</span></div>
                          <div className="mt-1 flex flex-col gap-1.5">
                            <div className="font-mono text-[20px] font-medium text-[#e8e8ed]">
                              {tokenSlider.toLocaleString()} <span className="text-[13px] text-[#9a9aaa] font-normal">tokens</span>
                            </div>
                            ...
                            <div className="flex justify-between text-[10px] text-[#9a9aaa] opacity-80 select-none">
                    ...
                    {calcTab === 'transcript' && (
                      <>
                        <div className="flex flex-col gap-2">
                          <div className="text-[11px] uppercase tracking-[0.06em] text-[#9a9aaa] opacity-70">Model</div>
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="text-[11px] uppercase tracking-[0.06em] text-[#9a9aaa] opacity-70">Recording duration</div>
                          <div className="flex mt-1 gap-1">
                        ...
                        {durIdx === 3 && (
                          <div className="flex flex-col gap-1.5">
                            <div className="text-[11px] uppercase tracking-[0.06em] text-[#9a9aaa] opacity-70">Exact duration (hours)</div>
                            <div className="flex overflow-hidden rounded-lg border border-white/[0.07]">
                              <div className="border-r border-white/[0.07] bg-[#1e1e27] px-3 py-2 text-[12px] text-[#9a9aaa] select-none">hrs</div>
                    ...
                    {calcTab === 'pipeline' && (
                      <>
                        <div className="flex flex-col gap-2">
                          <div className="text-[11px] uppercase tracking-[0.06em] text-[#9a9aaa] opacity-70">Transcription Model</div>
                          <div className="flex mt-1 gap-1">
                            {['turbo', 'prem'].map((m) => (
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="text-[11px] uppercase tracking-[0.06em] text-[#9a9aaa] opacity-70">Expected note tokens</div>
                          <div className="flex flex-col gap-1.5 mt-1">
                            <div className="font-mono text-[20px] font-medium text-[#e8e8ed]">
                              {pipeTokens.toLocaleString()} <span className="text-[13px] text-[#9a9aaa] font-normal">tokens</span>
                            </div>
                  ...
                  <div className="flex flex-col gap-3 border-l border-white/[0.07] pl-6">
                    <div className="flex flex-col gap-1 rounded-xl border border-white/[0.07] bg-[#18181f] px-4 py-4 surface-teal">
                      <div className="text-[10px] uppercase tracking-[0.08em] text-[#9a9aaa] opacity-90">Cost per generation</div>
                      <div className="flex items-center gap-2 my-1.5">
                        <span className="font-mono text-[36px] font-medium leading-none text-[#00d4c8]">{calcCost}</span>
                        <CreditIcon size={24} />
                      </div>
                      <div className="text-[12px] text-[#9a9aaa]">
                        {calcTierLabel} — <strong className="text-[#b4b4c2] font-medium">{calcTierRange}</strong>
                      </div>
                      {calcTab !== 'pipeline' && (
                        <div className="flex gap-1 mt-2">
                          {[0, 1, 2, 3].map((i) => (
                            <div key={i} className={`h-[3px] flex-1 rounded-full transition-colors duration-300 ${i <= calcTierIdx ? 'bg-[#00d4c8]' : 'bg-[#1e1e27]'}`} />
                          ))}
                        </div>
                      )}
                    </div>
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between rounded-lg border border-white/[0.07] bg-[#18181f] px-3 py-2.5">
                        <span className="text-[12px] text-[#9a9aaa]">Generations with <Cr amount={activeBalance} size={12} iconSize={11} /></span>
                        <span className="font-mono text-[13px] text-[#00d4c8]">{gens} {calcTab === 'pipeline' ? 'pipelines' : 'gens'}</span>
                      </div>
                      <div className="flex items-center justify-between rounded-lg border border-white/[0.07] bg-[#18181f] px-3 py-2.5">
                        <span className="text-[12px] text-[#9a9aaa]">Balance after this</span>
                        <Cr amount={balAfter} size={13} iconSize={12} className="text-[#e8e8ed]" />
                      </div>
                      {calcBreakdown && (
                        <div className="flex items-center justify-between rounded-lg border border-white/[0.07] bg-[#18181f] px-3 py-2.5">
                          <span className="text-[12px] text-[#9a9aaa]">Transcript + Notes</span>
                          <span className="flex items-center gap-1 font-mono text-[13px] text-[#e8e8ed]">
                            <Cr amount={calcBreakdown.t} size={13} iconSize={12} />
                            <span className="text-[#9a9aaa] mx-0.5">+</span>
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="flex flex-col gap-4 rounded-xl border border-white/[0.07] bg-[#111116] p-[18px] surface noise">
                <div>
                  <div className="mb-1 text-[13px] font-medium text-[#e8e8ed]">Credit packages</div>
                  <div className="text-[12px] leading-relaxed text-[#9a9aaa] max-w-[560px]">
                    Top up instantly with any major credit or debit card. Credits are added to your account <span className="text-[#e8e8ed] font-medium">immediately after payment</span>.
                  </div>
                </div>
                <div className="grid grid-cols-4 gap-2">
                  {TOPUP_PRESETS.map((p) => (
                    <button key={p.amount} onClick={() => setSelectedTopup(p.amount)}
                      ...
                      <div className={`flex items-center justify-center gap-1.5 font-mono text-[18px] font-medium ${selectedTopup === p.amount ? 'text-[#00d4c8]' : 'text-[#e8e8ed]'}`}>
                        {p.amount}
                        <CreditIcon size={16} color={selectedTopup === p.amount ? '#00d4c8' : '#e8e8ed'} />
                      </div>
                      <div className="text-[11px] text-[#9a9aaa] mt-0.5">{p.label} · {p.hint}</div>
                    </button>
                  ))}
                </div>
                <button onClick={() => router.push('/topup')}
                  className="flex w-fit items-center gap-2 rounded-lg bg-[#00d4c8] px-4 py-2.5 text-[13px] font-medium text-[#0c0c0e] transition-opacity hover:opacity-85">
                  <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 stroke-current fill-none stroke-2">
                    <path d="M12 5v14M5 12l7-7 7 7" />
                  </svg>
                  Top up {selectedTopup} credits
                </button>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="flex items-start gap-2.5 rounded-xl border border-white/[0.07] bg-[#111116] px-[18px] py-3.5">
              ...
              <p className="text-[12.5px] leading-relaxed text-[#9a9aaa]">
                Credits are deducted at generation time. A <strong className="text-[#b4b4c2] font-medium">balance check runs before every generation</strong> — if your credits are insufficient, <strong className="text-[#e8e8ed] font-medium">the request will not proceed</strong>.
                Token counts include both input and output. Top up instantly via card — credits land <span className="text-[#b4b4c2] font-medium">immediately after payment</span>.
              </p>
            </motion.div>
