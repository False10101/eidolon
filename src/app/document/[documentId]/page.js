    ...
    useEffect(() => {
        ...
        const fetchDocument = async () => {
            try {
                ...
                setName(data.name || "");
                setPromptTemplate(data.template_type === 'research_proposal' ? 0 :
                    data.template_type === 'business_proposal' ? 1 :
                        data.template_type === 'cover_letter' ? 2 :
                            data.template_type === 'formal_report' ? 3 :
                                data.template_type === 'general_essay' ? 4 : 0);

                setWordCount(wordCounter(data.user_prompt || 0));
                setCharCount(charCounter(data.user_prompt || 0));

                setTimeCreated(formatTimeAgo(data.created_at)); 
    ...
    return (
        <div className="flex h-full w-full justify-center items-center bg-gradient-to-r from-[#000120] to-[#18214E]">
            ...
            <div className="flex flex-col w-[21.5%] h-full border-r-[1px] border-white/[25%] p-4 bg-[#141B3C]/[64%]">
                ...
                <input type="text" className="w-full bg-black/[30%] border border-white/[25%] rounded-md text-white focus:outline-none focus:border-[#00BFFF] mt-3 mb-7 py-2 px-3 text-sm" placeholder="Enter document title" defaultValue={name}/>
            ...
            <div className="flex flex-col w-[39.5%] h-full border-r-[1px] border-white/[25%] bg-black/10 pb-3">
            ...
            <div className="flex flex-col w-[39.5%] h-full bg-[#1F2687]/[37%] ">
                <div className="w-full h-full bg-[#000831] flex flex-col pb-3">
                    ...
                    <div className="h-[7%] items-center border-t border-white/[25%] flex pt-3 space-x-2 px-3 "> 
                        <button className="flex w-[30%] bg-[#00CED1] px-2 py-2 text-white rounded-md hover:bg-[#0099CC] transition-colors flex items-center justify-center">
