...
export default function AdminPage() {
  ...
  <div className="px-4 py-3 flex flex-col gap-2">
    ...
    {['note', 'exam', 'transcript'].map((key) => {
      ...
      return (
        <div key={key} className="grid grid-cols-4 items-center rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] px-3 py-2 text-[11.5px]">
          ...
          <span className="text-right font-mono text-[var(--fg)]">${fmtFiat(data?.rev)}</span>
    ...
    <div className="flex items-center justify-between px-3 py-1.5 text-[11.5px]">
      <span className="text-[var(--fg-4)]">Total API Expenditure</span>
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      ...
                      {!usersLoading && (users ?? []).length === 0 && (
                        <tr><td colSpan={6} className="px-4 py-10 text-center text-[12px] text-[var(--fg-4)] opacity-40">No users found</td></tr>
                      )}
                      {(users ?? []).map(u => (
                        <tr key={u.id} className="border-b border-[var(--border-faint)] last:border-0 hover:bg-[var(--surface-tint-faint)] transition-colors">
