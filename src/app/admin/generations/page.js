    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[var(--bg)] font-sans text-sm text-[var(--fg)]">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <motion.div className="flex flex-1 flex-col gap-3 overflow-hidden px-7 py-5" variants={containerVariants} initial="hidden" animate="visible">
            <motion.div variants={itemVariants} className="flex flex-shrink-0 items-start justify-between gap-5">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-[var(--fg-4)]">Metadata and access receipts only. Generated content is never loaded.</p>
              </div>
              <div className="relative">
                ...
                <input value={search} onChange={event => setSearch(event.target.value)} placeholder="Title, generator, group..." className="w-[280px] rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] py-1.5 pl-9 pr-3 text-[12px] text-[var(--fg)] outline-none placeholder:text-[var(--fg-4)] focus:border-[rgba(0,212,200,0.35)]" />
              </div>
            </motion.div>

            <motion.div variants={itemVariants} className="grid flex-shrink-0 grid-cols-4 gap-2.5">
              {cards.map(card => (
                <div key={card.label} className="surface rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3.5 py-2.5">
                  <div className="text-[9.5px] uppercase tracking-[0.08em] text-[var(--fg-4)] opacity-55">{card.label}</div>
                  <div className="mt-1 text-[18px] font-medium text-[var(--fg)]">
            ...
            <motion.div variants={itemVariants} className="flex flex-shrink-0 items-center justify-between">
              <div className="flex rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] p-1">
                {[['all', 'All'], ['note', 'Notes'], ['transcript', 'Transcripts']].map(([value, label]) => (
                  <button key={value} onClick={() => setType(value)} className={`rounded px-3 py-1 text-[10.5px] transition-all ${type === value ? 'bg-[var(--surface-deep)] text-[var(--fg)] shadow-sm' : 'text-[var(--fg-4)] hover:text-[var(--fg-3)]'}`}>{label}</button>
                ))}
              </div>
              <div className="flex rounded-lg border border-[var(--border)] bg-[var(--surface-raised)] p-1">
                  ...
                  <thead>
                    <tr>
                      {['Generation', 'Mode', 'Generator / Group', 'Access', 'Per user', 'Collected', 'Metadata', 'Status', 'Created'].map(heading => (
                        <th key={heading} className="sticky top-0 z-10 border-b border-[var(--border)] bg-[var(--surface)] px-3 py-2.5 text-left text-[9.5px] font-normal uppercase tracking-[0.08em] text-[var(--fg-4)] opacity-60">{heading}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {loading && Array.from({ length: 7 }).map((_, rowIndex) => (
                      <tr key={rowIndex} className="border-b border-[var(--border-faint)]">
                        {Array.from({ length: 9 }).map((__, columnIndex) => <td key={columnIndex} className="px-3 py-2.5"><Skeleton width={columnIndex === 0 ? 'w-32' : 'w-16'} /></td>)}
                      </tr>
                    ))}
                    {!loading && generations.length === 0 && !error && (
                      <tr><td colSpan={9} className="px-3 py-10 text-center text-[11px] text-[var(--fg-4)] opacity-50">No matching generations found</td></tr>
                    )}
                            ...
                            <td className="px-3 py-2.5">
                              <div className="flex max-w-[240px] items-center gap-2">
                                ...
                                <div className="min-w-0">
                                  <div className="truncate text-[12px] font-medium text-[var(--fg)]" title={item.title}>{item.title || 'Untitled'}</div>
                                  <div className={`mt-0.5 text-[9.5px] uppercase tracking-[0.08em] ${item.resourceType === 'note' ? 'text-[var(--accent)]' : 'text-[#8b5cf6]'}`}>{item.resourceType}</div>
                                </div>
                              </div>
                            </td>
                            <td className="px-3 py-2.5"><span className={`rounded-full border px-2 py-0.5 text-[9.5px] capitalize ${isGroup ? 'border-[rgba(0,212,200,0.2)] bg-[rgba(0,212,200,0.07)] text-[var(--accent)]' : 'border-[var(--border)] bg-[var(--surface-raised)] text-[var(--fg-4)]'}`}>{item.generationType || 'individual'}</span></td>
                            <td className="px-3 py-2.5">
                              <div className="text-[11.5px] text-[var(--fg-2)]">{item.generator?.username || item.generator?.email || `User ${item.generator?.userId}`}</div>
                              <div className="mt-0.5 max-w-[170px] truncate text-[10px] text-[var(--fg-4)]">{item.group?.name || item.generator?.email || '—'}</div>
                            </td>
                            <td className="px-3 py-2.5">
                              {isGroup ? <><div className="text-[11.5px] text-[var(--fg)]">{item.unlocks.total} unlocked</div><div className="mt-0.5 text-[10px] text-[var(--fg-4)]">{item.unlocks.original} original · {item.unlocks.later} later</div></> : <span className="text-[10.5px] text-[var(--fg-4)]">Owner only</span>}
                            </td>
                            <td className="px-3 py-2.5">
                              {perPerson ? <><div className="inline-flex items-center gap-1 text-[11.5px]"><CreditIcon className="h-3 w-3" />{perPerson}</div>{item.paid.nextUnlock != null && <div className="mt-0.5 text-[9.5px] text-[var(--fg-4)]">next {fmtCredit(item.paid.nextUnlock)}</div>}</> : <span className="text-[10.5px] text-[var(--fg-4)]">—</span>}
                            </td>
                            <td className="px-3 py-2.5 text-[11.5px]"><CreditAmount value={collected} /></td>
                            <td className="px-3 py-2.5">
                              {item.resourceType === 'note' ? <><div className="text-[11px] capitalize text-[var(--fg-3)]">{item.metadata.style || 'Default style'}</div><div className="mt-0.5 text-[9.5px] text-[var(--fg-4)]">{item.metadata.totalTokens == null ? '—' : `${Number(item.metadata.totalTokens).toLocaleString()} tokens`}</div></> : <><div className="text-[11px] text-[var(--fg-3)]">{formatDuration(item.metadata.durationSeconds)}</div><div className="mt-0.5 max-w-[120px] truncate text-[9.5px] text-[var(--fg-4)]" title={item.metadata.model}>{item.metadata.model || '—'}</div></>}
                            </td>
                            <td className="px-3 py-2.5"><span className={`text-[10px] capitalize ${['completed', 'success', 'ready'].includes(String(item.status).toLowerCase()) ? 'text-[#22c55e]' : 'text-[var(--fg-4)]'}`}>{item.status || '—'}</span></td>
                            <td className="whitespace-nowrap px-3 py-2.5 text-[10px] text-[var(--fg-4)]">{formatDate(item.createdAt)}</td>
                          </tr>
                                      ...
                                      {item.participants.length === 0 ? (
                                        ...
                                        <div className="grid grid-cols-1 gap-2 xl:grid-cols-2">
                                          {item.participants.map(participant => (
                                            <div key={participant.userId} className="flex items-center justify-between rounded-lg border border-[var(--border-faint)] bg-[var(--surface)] px-3 py-2">
                                              <div className="min-w-0">
                                                <div className="flex items-center gap-2"><span className="truncate text-[11px] text-[var(--fg-2)]">{participant.username || `User ${participant.userId}`}</span><span className={`rounded px-1.5 py-0.5 text-[8.5px] uppercase tracking-[0.06em] ${participant.isOriginal ? 'bg-[rgba(0,212,200,0.08)] text-[var(--accent)]' : 'bg-[rgba(139,92,246,0.1)] text-[#8b5cf6]'}`}>{participant.isOriginal ? 'Original' : 'Later'}</span></div>
                                                <div className="mt-0.5 truncate text-[9.5px] text-[var(--fg-4)]">{participant.email || `User ID ${participant.userId}`} · {formatDate(participant.unlockedAt)}</div>
                                              </div>
                                              <div className="ml-4 flex-shrink-0 text-[11px]"><CreditAmount value={participant.paidAmount} /></div>
                                            </div>
                                          ))}
                                        </div>
                                      )}
