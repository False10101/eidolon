...
// ─── Page ──────────────────────────────────────────────────────────────────────
export function ExamPrepViewerLegacy() {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setUnlockError(d.error ?? t('errorUnlockFailed'));
        return;
      }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {!loading && data && (
          ...
          >
                  ...
                  <div className="rounded-xl border border-[rgba(0,212,200,0.1)] bg-[var(--surface)] overflow-hidden surface-teal">
                    <div className="flex flex-col divide-y divide-[var(--border-faint)]">
                      {[
                        ...
                        { label: t('cost'),      value: `${Number(data.charge_amount).toFixed(2)}`, cyan: true, costCredits: data.charge_amount },
                      ].map(({ label, value, cyan, costCredits }) => (
                        <div key={label} className="flex items-center justify-between px-3.5 py-2.5 text-[12px]">
                          <span className="text-[var(--fg-3)]">{label}</span>
                          <span className={`font-mono text-[11.5px] ${cyan ? 'text-[var(--accent)]' : 'text-[var(--fg)]'}`}>
                            {value} {costCredits != null ? <CreditIcon size={12}/> : ""} {costCredits != null && <LocalCreditPrice credits={costCredits} />}
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            {/* Locked Overlay */}
            {!data.is_unlocked && (
              <div className="absolute inset-0 z-50 flex items-center justify-center bg-[var(--bg)]/80 backdrop-blur-md">
                <div className="flex flex-col items-center p-8 text-center bg-[var(--surface)] border border-[var(--border)] rounded-2xl max-w-[400px]">
                  ...
                  <h2 className="text-[18px] font-semibold text-[var(--fg)] mb-2">Locked Group Exam Prep</h2>
                  <p className="text-[13px] text-[var(--fg-3)] mb-6">
                    This exam prep was generated by your group. You can unlock it for {data.charge_amount} credits{' '}
                    <LocalCreditPrice credits={data.charge_amount} /> to view its contents.
                  </p>
                  {unlockError && (
                  ...
                  >
                    {unlocking ? (
                      ...
                      <>Unlock for {data.charge_amount} <CreditIcon size={13} /> <LocalCreditPrice credits={data.charge_amount} /></>
                    )}
                  </button>
                </div>
              </div>
