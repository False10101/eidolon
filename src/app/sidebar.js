...
const NAV_ITEMS = [
  ...
  { labelKey: 'examPrep',       href: '/exam-prep', comingSoon: true, icon: <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 stroke-current fill-none stroke-[1.8]"><path d="M9 11l3 3L22 4" /><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" /></svg> },
...
const HISTORY_CONFIG = {
  ...
  '/transcriptor':{ api: '/api/transcript/getHistory', newHref: '/transcriptor/new', newLabelKey: 'newTranscript', detailBase: '/transcriptor', hasGroupSplit: true },
...
function HistoryItem({ item, active, onClick }) {
  const isPending = item.status && !['completed', 'failed'].includes(item.status.toLowerCase());
  ...
  return (
    <button
      ...
      className={`group flex w-full items-center gap-2.5 border-l-2 px-4 py-2 text-left transition-all duration-100
        ${isPending ? 'opacity-60 cursor-default border-transparent' : ''}
        ${!isPending && active
          ? 'border-[var(--accent)] bg-[rgba(0,212,200,0.07)]'
          : !isPending ? 'border-transparent hover:bg-[var(--surface-hover)] hover:border-[var(--border)]' : ''
        }`}
    >
      {isPending ? (
        <div className="flex-shrink-0 flex h-3 w-3 -ml-[3px] items-center justify-center">
          <div className="h-2.5 w-2.5 animate-spin rounded-full border-[1.5px] border-[var(--fg-4)] border-t-transparent" />
        </div>
      ) : (
        <div className={`h-[5px] w-[5px] flex-shrink-0 rounded-full transition-colors ${active ? 'bg-[var(--accent)]' : 'bg-[var(--scrollbar)]'}`} />
      )}
      <div className="flex-1 min-w-0 flex items-center gap-2">
        <span className={`truncate text-[12.5px] transition-colors ${!isPending && active ? 'text-[var(--fg)]' : 'text-[var(--fg-3)]'} ${!isPending ? 'group-hover:text-[var(--fg)]' : ''}`}>
          {item.label ?? item.name}
        </span>
        {item.isGroup && (
          <span className="flex-shrink-0 rounded border border-[rgba(0,212,200,0.15)] bg-[rgba(0,212,200,0.04)] px-1 py-[1px] text-[8px] uppercase tracking-[0.05em] text-[var(--fg-4)]">
            Grp
          </span>
        )}
      </div>
...
export default function Sidebar() {
  ...
  const config = baseRoute === '/exam-prep' ? null : (HISTORY_CONFIG[baseRoute] ?? null);

  // On list pages (/note, /exam-prep), we don't show history — just the nav
  const isListPage = pathname === '/note' || pathname === '/exam-prep' || pathname === '/transcriptor';
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        const combined = [
          ...(data.group || []).map(item => ({ ...item, isGroup: true })),
          ...(data.individual || []).map(item => ({ ...item, isGroup: false }))
    ...
    fetchHistory();
  }, [baseRoute, config, getAccessTokenSilently, isDetailPage, showHistory]);
      ...
      <nav className="flex-shrink-0 border-b border-[var(--border)] py-2">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button
              key={item.href}
              onClick={item.comingSoon ? undefined : () => router.push(item.href)}
              disabled={item.comingSoon}
              className={`flex w-full items-center gap-2.5 border-l-2 px-4 py-[7px] text-[13px] transition-all duration-150
                ${item.comingSoon
                  ? 'cursor-not-allowed border-transparent text-[var(--fg-4)] opacity-65'
                  : active
            ...
            >
              ...
              {item.comingSoon && (
                <span className="ml-auto rounded border border-[rgba(0,212,200,0.18)] bg-[rgba(0,212,200,0.05)] px-1.5 py-0.5 text-[8px] font-medium uppercase tracking-[0.06em] text-[var(--accent-muted)]">
                  Coming Soon
                </span>
              )}
            </button>
      ...
      {config && (
        <div className="flex-shrink-0 border-b border-[var(--border)] p-3">
          <button
            ...
            className="btn-accent flex w-full items-center justify-center gap-2 rounded-lg py-2 text-[13px] font-medium transition-all"
          >
