...
export async function POST(req) {
    ...
    try {
        ...
        // ---------------------------------------------------------
        //  3. UNIVERSAL RENDERER (Safe for v12+)
        // ---------------------------------------------------------
        marked.use({
            renderer: {
        ...
        if (toc.length > 0) {
            ...
            toc.forEach(item => {
                if (item.level <= 4) { // Includes Level 4
        ...
        // ---------------------------------------------------------
        //  5. PUPPETEER (Cross-Platform)
        // ---------------------------------------------------------
        let browser;
        
        // Detect Localhost (Mac/Windows) vs VPS (Linux)
        ...
        // Only trigger Spartan update if on Linux
        if (process.platform !== 'darwin' && process.platform !== 'win32') {
