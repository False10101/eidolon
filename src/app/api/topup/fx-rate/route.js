...
const FX_CACHE_TTL  = 60 * 60 * 24;      // 24h — exchange rate
const GEO_CACHE_TTL = 60 * 60 * 24 * 7;  // 7d  — IP → country
...
const USD_RESPONSE = { currency: 'USD', symbol: '$', rate: 1, locale: 'en-US' };

export async function GET(req) {
    // ── Dev override ──────────────────────────────────────────────────────────
    // Set DEV_COUNTRY=TH in .env.local to test local currency display locally
    if (process.env.NODE_ENV === 'development' && process.env.DEV_COUNTRY) {
        const meta = COUNTRY_CURRENCY[process.env.DEV_COUNTRY];
        if (!meta) return NextResponse.json(USD_RESPONSE);
        const rate = await getFxRate(meta.currency);
        if (!rate) return NextResponse.json(USD_RESPONSE);
        return NextResponse.json({ ...meta, rate });
    }

    // ── Production: resolve from real IP ─────────────────────────────────────
    const ip = req.headers.get('x-real-ip')
        ?? req.headers.get('x-forwarded-for')?.split(',')[0].trim()
        ?? null;

    if (!ip) return NextResponse.json(USD_RESPONSE);
