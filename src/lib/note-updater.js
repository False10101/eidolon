...
export async function updateNoteInBackground(noteId) {
    ...
    try {
        ...
        const absoluteNotePath = path.join(process.cwd(), 'storage', note.noteFilePath);
        
        await writeFile(absoluteNotePath, generatedText, 'utf-8');

        await db.query(`UPDATE note SET status = 'COMPLETED' WHERE id = ?`,
            [noteId]
        );
